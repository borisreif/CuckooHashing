/**
 * 
 * Bucketed cuckoo mapg using one flat 1D array implemented as a factory
 * 
 * 
 * @author Boris A. Reif
 * @version 0.0.2
 * 
 * 
 * @links https://www.brics.dk/RS/01/32/BRICS-RS-01-32.pdf
 * @links https://www.geeksforgeeks.org/dsa/cuckoo-hashing/
 * @links https://codecapsule.com/2013/07/20/cuckoo-hashing/
 * @links https://en.wikipedia.org/wiki/Cuckoo_hashing
 * 
 * 
 * 
 * Logical model:
...
function createConfig({
    ...
    for (const hashIndex of tableToHash) {
        if (!Number.isInteger(hashIndex) || hashIndex < 0 
                                         || hashIndex >= hashFunctions.length) {
            throw new Error(
                "tableToHash contains an invalid hash-function index"
            );
        }
    }
...
function printTable(table) {
    ...
    cfg.logger(table, "Final bucketed cuckoo tables:\n");

    for (let tableIdx = 0; tableIdx < cfg.numTables; tableIdx++) {
        cfg.logger(table, `Table ${tableIdx}:`);
        ...
        for (let bucketIdx = 0; bucketIdx < cfg.bucketCount; bucketIdx++) {
            ...
            cfg.logger(table, `  Bucket ${bucketIdx}: [ ${values.join(" ")} ]`);
        }
    ...
    cfg.logger(table, "Underlying flat 1D array:");
    cfg.logger(table, flat.join(" "));
    cfg.logger(table, "");
}
...
function cuckoo(table, keys) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (!inserted) {
            cfg.logger(table, `Failed to insert key ${keys[i]}. Rehash needed.`);
            break;
        }
    }
 ...
 * @param {Object} table
 * @param {number} key
 * @returns {boolean} True if the key was removed, false if it was not found.
 */
function remove(table, key) {
    ...
    table.cells[result.flatIndex] = table.config.empty;
    return true;
}
