 ...
 * - "sameValueZero" (default)
 * - "objectIs"
 *
 * Notes:
 * - "sameValueZero" matches JavaScript Map semantics:
 *   - NaN equals NaN
 *   - 0 equals -0
 * - "objectIs" is more precise:
 *   - NaN equals NaN
 ...
 * @param {"sameValueZero"|"objectIs"} [options.equality="sameValueZero"]
 * @returns {{hashBucket: Function, equals: Function, formatKey: Function}}
 */
export function createStringNumberKeyOps({ equality = "sameValueZero" } = {}) {
  const seeds = [0x00000000, 0x9e3779b9, 0x85ebca6b, 0xc2b2ae35, 0x27d4eb2f];
  ...
  switch (equality) {
    ...
    default:
      throw new Error(`Unsupported equality mode: ${String(equality)}`);
  }
  ...
  return {
    ...
    hashBucket(key, which, bucketCount) {
      ...
      const seed =
        seeds[which % seeds.length] ^ Math.imul(which + 1, 0x9e3779b1);
      const mixed = which === 0 ? h : mix32(h ^ seed);
      return mixed % bucketCount;
    },

    /**
     * Compare two keys using the configured equality mode.
     *
     * @param {*} a
     * @param {*} b
     * @returns {boolean}
